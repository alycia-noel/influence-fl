...
def pert_grad(args, device, model, possible, epsilon, train_df, train_df_rr):
    ...
    for (index, row), (index_rr, row_rr) in zip(train_df.iterrows(), train_df_rr.iterrows()):
        ...
        original_randomized_label = torch.FloatTensor([original_randomized_data["Grade"]]).to(device).to(torch.int64)
        ...
        for i, combo in enumerate(cartesian):
            ...
            randomized_label = torch.FloatTensor([randomized_data["Grade"]]).to(device).to(torch.int64)
...
def do_influence(args):
    ...
    #     print(k)
        # train_df_rr_y = torch.FloatTensor(train_df_rr_clients[k]['Grade'].values).to(device).to(torch.int64)
        # train_df_rr_x = torch.FloatTensor(train_df_rr_clients[k].drop(['Grade'], axis=1).values).to(device)
        # full_test_y = torch.FloatTensor(full_test_df['Grade'].values).to(device).to(torch.int64)
        # full_test_x = torch.FloatTensor(full_test_df.drop(['Grade'], axis=1).values).to(device)
    ...
    train_df_rr_y = torch.FloatTensor(train_df_rr_clients[0]['Grade'].values).to(device).to(torch.int64)
    train_df_rr_x = torch.FloatTensor(train_df_rr_clients[0].drop(['Grade'], axis=1).values).to(device)
    full_test_y = torch.FloatTensor(full_test_df['Grade'].values).to(device).to(torch.int64)
    full_test_x = torch.FloatTensor(full_test_df.drop(['Grade'], axis=1).values).to(device)
    removal_model, randomized_model, influence_loss, influence_randomize = calc_influence(args, device, deepcopy(global_model), [train_df_rr_x, train_df_rr_y], train_df_rr_clients[0], train_df_clients[0], [full_test_x, full_test_y], weights, possible, args.change_epsilon_value, 0)
    ...
    test_loss_rem, test_acc_rem = test_inference(removal_model, [testloader])
    print(test_loss, test_acc)
    print(test_loss_rem, test_acc_rem)
    return removal_model.state_dict(), randomized_model.state_dict(), test_loss, test_acc, test_loss_rem, test_acc_rem, influence_loss
...
def main(args):
    ...
    if args.save == "yes":
        ...
        with open(f'/home/ancarey/influence/{args.dataset}_data_{args.epsilon}_{args.round}_multiple_{args.how_many}.pkl', 'wb') as f:  
    ...
    if args.do_diff_rr == "yes":
        for k in range(args.how_many):
            ...
            randomized_train_pd = PandasDataset(randomized_train_df, 'glioma')
    ...
    # if args.save == "yes":
    #     torch.save(global_model, f'/home/ancarey/influence/{args.dataset}_global_model_epsilon_{args.epsilon}_{args.round}_removal.pt')
...
if __name__ == '__main__':
    ...
    rem_loss, rem_acc, rem_param = {i:[] for i in change_epsilon_values[args.epsilon]}, {i:[] for i in change_epsilon_values[args.epsilon]}, {i:[] for i in change_epsilon_values[args.epsilon]}
    ...
    for r in range(num_to_do):
        ...
        for ce in change_epsilon_values[args.epsilon]:
            ...
            removal_model, randomized_model, test_loss, test_acc, test_loss_rem, test_acc_rem, randomized_influence_loss = do_influence(args)
            ...
            rem_param[ce].append(list(removal_model.values())[0].detach().cpu().numpy())
            rem_loss[ce].append(round(test_loss_rem[0], 6))
            rem_acc[ce].append(round(test_acc_rem[0]*100, 4))
    ...
    to_save_dict = {'normal_losses': normal_losses,
                    ...
                    'rem_losses': rem_loss, 
                    'rem_accs': rem_acc, 
                    'rem_params': rem_param,
                    'pert_inf': pert_influence,
                }
    with open(f'/home/ancarey/influence/{args.dataset}_data_{args.epsilon}_results_redo_changesign.pkl', 'wb') as f:  
