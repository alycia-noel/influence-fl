...
def calc_influence(args, device, model, train_data_rr, train_data_rr_df, train_data, test_data, weights, possible, epsilon, which_client):
    ...
    influence_randomize = torch.mm(est_hess.to(device), grad_randomized[0].T) * weights[0] * (1/len(train_data_rr[0]))
    ...
    influence_loss = torch.dot(ihvp.flatten(), grad_randomized[0].flatten()) * (1/len(train_data_rr[0])) * weights[0]
...
def pert_grad(args, device, model, possible, epsilon, train_df, train_df_rr):
    ...
    model.eval()
    sensitive_columns = ['RAC1P', "SEX", "PINCP"]
    ...
    for (index, row), (index_rr, row_rr) in zip(train_df.iterrows(), train_df_rr.iterrows()):
        ...
        original_randomized_label = torch.FloatTensor([original_randomized_data["PINCP"]]).to(device).to(torch.int64)
        ...
        for i, combo in enumerate(cartesian):
            ...
            randomized_label = torch.FloatTensor([randomized_data["PINCP"]]).to(device).to(torch.int64)
...
def do_influence(args):
    ...
    train_df_rr_y = torch.FloatTensor(train_df_rr_clients[0]['PINCP'].values).to(device).to(torch.int64)
    train_df_rr_x = torch.FloatTensor(train_df_rr_clients[0].drop(['PINCP'], axis=1).values).to(device)
    full_test_y = torch.FloatTensor(full_test_df['PINCP'].values).to(device).to(torch.int64)
    full_test_x = torch.FloatTensor(full_test_df.drop(['PINCP'], axis=1).values).to(device)
           
    # randomized_model_est, randomized_influence_loss = calc_influence(args, device, deepcopy(global_model), [train_df_rr_x, train_df_rr_y], train_df_rr_clients[0], train_df_clients[0], [full_test_x, full_test_y], weights, possible, args.change_epsilon_value)
    ...
    test_loss, test_acc = test_inference(removal_model, [testloader])

    return removal_model.state_dict(), test_loss, test_acc, randomized_influence_loss
...
def main(args):
    ...
    #     train_df_to_randomize = copy.deepcopy(train_df_clients[0])
    ...
    #     train_df_rr_clients[0] = randomized_train_df
    #     randomized_train_pd = PandasDataset(randomized_train_df, 'adult')
    #     randomized_trainloader = DataLoader(randomized_train_pd, batch_size=args.local_bs, shuffle=True)
    #     train_loaders[0] = randomized_trainloader
    ...
    # if args.save == "yes":
    #     torch.save(global_model, f'/home/ancarey/influence/{args.dataset}_global_model_epsilon_{args.epsilon}_{args.round}.pt')
...
if __name__ == '__main__':
    ...
    with open(f'/home/ancarey/influence/{args.dataset}_data_{args.epsilon}_results_redo_inf_calc.pkl', 'wb') as f:  
